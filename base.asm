;משחק בלאקג'ק- blackjack fantasy
;נוצר ע"י איל רייפלר
;
;משחק זה הוא וריאציה של המשחק בלאקג'ק, שבו אתה, המשתמש, משחק נגד המחשב(הדילר).
;יש ברשותך בכל תור שתי אפשרויות במשחק:
;HIT או STAND
;HIT:
;יביא לך עוד קלף מתוך חפיסה של 52 קלפים.
;STAND:
;יעביר את התור לדילר.
;
;על לוח המשחק יוצגו סך הנקודות של המשתמש והדילר, ערך היד של המשתמש והדילר, אילו קלפים יצאו לכל אחד
;
;מטרת המשחק היא להגיע לחמש נקודות, לפני הדילר:
;
;אם ערך היד שלך גבוה משל הדילר, ולא עובר את 21, קיבלת נקודה.
;אם ערך היד שלך הוא בדיוק 21, קיבלת שתי נקודות.
;אם ערך היד שלך עבר את 21, הפסדת.
;אם ערך היד של הדילר גבוה משלך, וקטן מ21, הפסדת.
;
;Procuders:)
; אלה הפרוצדורות שחשבתי שיהיו. לא עדכניות
;        ✓ 1)  Title_Screen             | - מדפיס את המסך פתיחה
;          2)  Beginning                | - מאתחל את הנקודות לאפס ומאפשר להיכנס למשחק או להוראות
;          3)  Game_Or_Instructions         | - מציג תמונה של ההוראות ומאפשר להגיע חזרה למסך פתיחה או להגיע למשחק
;          5)  Start_Hand               | - לאתחל את היד ואת חפיסת הקלפים
;          4)  Draw_Crad                | - להוסיף קלף למשתמש או לדילר
;          5)  Value_Hand               | - לחשב את ערך היד הכולל
;          6)  Card_Voice               | - להשמיע צליל של חלוקת קלף
;       ✓  7)  wait075sec               | - לעשות דיליי של 0.75 שנייה בשביל חלקות הקלפים  
;          8)  Show_Game                | - לעשות את הלוח משחק
;                9)  Check_ Win         | - לבדוק אם יש בלאג'ק או אם עבר את 21 הנקודות
;                10)  Print_Hand        | - להדפיס את כל הקלפים בכל יד בשורה
;      11)  Face_Cards                  | - פרוצדורה שעוזרת להציג את הערך של קלף תמונה
;      12) Card_Spacer                  | - עושה רווח בין קלף לקלף
;      13) Player_Turn                  | - 'המשתמש יכול לבחור אם ללחוץ על 'היט' או 'סטאנד
;      14) Dealer_Turn                  | - 'הדילר יכול לבחור אם ללחוץ על 'היט' או 'סטאנד
;      15) Choose_Winner                | - אם גם המשתמש וגם הדילר עושים סטאנד, לראות למי יש יותר נקודות
;      16) Game_Over                    | - להציג מסך נצחון או הפסד
;      17) Exit_Game                    | - לתת למשתמש לבחור אם לצאת מהמשחק למסך או לעשות עוד משחק
;
;
;בעיות שיש בינתיים:
;אין יותר בעיות שמצאתי
; 
;
;
;
